...
public class DownloadTwitterTask extends AsyncTask<String, Void, String> {
	...
	public DownloadTwitterTask(final Activity activity) {
	...
	protected String doInBackground(final String... screenNames) {
		...
		if (screenNames.length > 0) {
			result = this.getTwitterStream(screenNames[0]);
		}
		return result;
	}
	...
	protected void onPostExecute(final String result) {
		Twitter twits = this.jsonToTwitter(result);

		// send the tweets to the adapter for rendering

		ArrayAdapter<Tweet> adapter = new ArrayAdapter<Tweet>(this.activity, android.R.layout.simple_list_item_1, twits);
		final ListView listview = (ListView) this.activity.findViewById(R.id.listview);
		listview.setAdapter(adapter);
	}

	// converts a string of JSON data into a Twitter object
	private Twitter jsonToTwitter(final String result) {
		Twitter twits = null;
	...
	private String getTwitterStream(final String screenName) {
		...
		try {
			...
			Authenticated auth = this.jsonToAuthenticated(rawAuthorization);
